...
public class MyBedFragment extends BaseFragment implements View.OnClickListener, CompoundButton.OnCheckedChangeListener{
    ...
    private void initView() {
        ...
        HashMap<String, String> hashMap = AppConfig.getInstance().getMyBed();
        if (hashMap == null){
            hashMap = new HashMap<>();
        }

        mLinearMyBed = (LinearLayout) mRootView.findViewById(R.id.linear_my_bed);
        mBtnOk = (Button) mRootView.findViewById(R.id.btn_bed_ok);
    ...
    public void onClick(View v) {
        ...
        switch (v.getId()){
            case R.id.btn_bed_ok:
                // 确定改变
                AppConfig.getInstance().setMyBed(CommonUtil.saveAsHashMap(mCheckList)); // 将选择的床位保存在本地
                break;
            case R.id.btn_bed_all:
                // 选择全部
                if (mCheckList != null){
